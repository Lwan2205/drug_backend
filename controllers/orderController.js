...
const createOrderFromCart = async (req, res, next) => {
    try {
        ...
        const cart = await Cart.findOne({ userId: req.user._id }).populate({
            ...
            select: 'price manufacturer discount stock', // Cần lấy thêm trường stock
        ...
        const updateStockPromises = cart.products.map(async item => {
            ...
            const newStock = product.stock - item.quantity; // Trừ số lượng trong kho
            if (newStock < 0) {
...
const updateOrderStatus = async (req, res) => {
    ...
    try {
        ...
        // Lưu trạng thái đơn hàng đã cập nhật
