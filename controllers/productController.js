...
const getProductDiscount = async (req, res) => {
    try {
        ...
        const discounts = await Discount.find({
            'endDate': { $gte: currentDate } // Chỉ lấy discount có endDate lớn hơn hoặc bằng ngày hiện tại
        });

        // Kiểm tra nếu không có discount còn hạn
        if (!discounts || discounts.length === 0) {
            return res.status(404).json({ success: false, message: 'No active discounts found.' });
        }




        const productIds = discounts.map(discount => discount._id); // Lấy mảng các discount IDs

        const products = await Product.find({
            discount: { $in: productIds } // Lọc các sản phẩm có discount còn hạn
        })
            .populate('discount', 'name discountPercent endDate') // Populate để lấy thông tin discount
            .sort({ totalSold: -1 })
            .limit(5);


        if (!products || products.length === 0) {
            return res.status(404).json({ success: false, message: 'No products with active discounts found.' });
        }

        res.status(200).json({ success: true, products });

    } catch (error) {
        console.error('Error fetching products with active discounts:', error);
...
const getRelatedProduct = async (req, res) => {
    ...
    try {
        ...
        // Lấy các sản phẩm liên quan dựa trên cùng thương hiệu, loại, hoặc tiêu chí khác
...
const getAllProductDiscount = async (req, res) => {
    ...
    try {
        ...
        // Bước 2: Lấy các productIds từ discount
        const productIds = discounts.map(discount => discount._id); // Lấy mảng các discount IDs

        // Bước 3: Truy vấn các sản phẩm có discount còn hạn
        const products = await Product.find({
            discount: { $in: productIds } // Lọc các sản phẩm có discount còn hạn
        })
            .skip(skip)
        ...
        const totalProducts = await Product.countDocuments({
            discount: { $in: productIds } // Lọc tổng số sản phẩm có discount còn hạn
        });

        const totalPages = Math.ceil(totalProducts / limit); // Tính số trang tổng cộng
